...
# Dist_J = 4.9501*AU                          # Distance of Jupiter from Sun
...
print(vR)
# vR = np.array([0,-np.sqrt(G*M/Pos_E[0])-np.sqrt(G*Mass_E/7000e3)])
...
class planet:
    ...
    def __init__(self, name, m, r, v):
        ...
        if self.name == 'Earth':
            self.r_array.append(Pos_E)
            self.v_array.append(Vel_E)
        if self.name == 'Jupiter':
            self.r_array.append(Pos_J)
            self.v_array.append(Vel_J)
        if self.name == 'Saturn':
            self.r_array.append(Pos_S)
            self.v_array.append(Vel_S)
...
class rocket:
    ...
    def __init__(self, mR, vR):
        ...
        self.rR3 = 0
        self.rR3_array = []
        self.r = np.array([Pos_E[0]+7000e3, 0]) # Distance from sun
        self.r_array = []
        self.m = mR
        self.F = np.array([0, 0])
        ...
        while t < N:
            ...
            r_mag = np.sqrt(self.r.dot(self.r))
            r_hat = self.r/r_mag
            self.F = -G*M*self.m*r_hat/self.r.dot(self.r) #Force due to sun
            
            #Force due to earth
            self.rR1 = self.r-earth.r_array[t]
            rR1_mag = np.sqrt(self.rR1.dot(self.rR1))
            self.rR1_array.append(rR1_mag)
            rR1_hat = self.rR1/rR1_mag
            # self.F1 = -G*Mass_E*self.m*rR1_hat/self.rR1.dot(self.rR1)
            ...
            self.F = self.F + self.F1 + self.F2 + self.F3 + self.F4
            self.F_array.append(self.F)
...
def animate(i):
    j = int(N * i / framerate)
    # if rocket.rR1_array[j] < 2e10: time.sleep(0.4)
    ...
    # Prespective of the rocket
    # plt.xlim([x-7000e4, x+7000e4])
    # plt.ylim([y-7000e4, y+7000e4])
